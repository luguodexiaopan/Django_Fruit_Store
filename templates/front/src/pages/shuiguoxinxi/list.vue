...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"12px 10px 20px","margin":"20px 0 0 0","borderColor":"#52b646","alignItems":"center","borderRadius":"12px","flexWrap":"wrap","background":"#eff9ee","borderWidth":"4px","display":"flex","width":"100%","borderStyle":"solid dashed dotted double","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>水果名称</div>
        <el-input v-model="formSearch.shuiguomingcheng" placeholder="水果名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>水果分类</div>
        <el-select v-model="formSearch.shuiguofenlei" placeholder="请选择水果分类" :clearable="true">
          <el-option v-for="(item, index) in shuiguofenleiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>季节</div>
        <el-select v-model="formSearch.jijie" placeholder="请选择季节" :clearable="true">
          <el-option v-for="(item, index) in jijieOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	  ...
	  <el-button v-if="isAuth('shuiguoxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(255,215,169,1) 0%, rgba(255,164,58,1) 100%)","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/shuiguoxinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"margin":"20px 0 20px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"10px 10px","borderColor":"#52b646","borderRadius":"8px","flexWrap":"wrap","background":"#fff","borderWidth":"4px","display":"flex","width":"100%","borderStyle":"solid double solid double","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"border":"2px solid #88d67e","cursor":"pointer","padding":"8px","boxShadow":"-1px 2px 4px rgba(0,0,0,.1)","margin":"10px","borderRadius":"8px","background":"#fff","display":"inline-block","width":"23%","position":"relative","borderStyle":"dotted dashed solid double","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				...
				<div :style='{"padding":"0","lineHeight":"30px","fontSize":"14px","color":"#333"}' class="name ">{{item.shuiguomingcheng}}</div>
				<div :style='{"padding":"0","lineHeight":"30px","fontSize":"14px","color":"#333"}' class="name ">{{item.shuiguofenlei}}</div>
				<div :style='{"padding":"0","lineHeight":"30px","fontSize":"14px","color":"#333"}' class="name ">季节:{{item.jijie}}</div>
				<div :style='{"padding":"0","lineHeight":"30px","fontSize":"14px","color":"#333"}' class="name ">品牌:{{item.pinpai}}</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/shuiguofenlei/shuiguofenlei').then(res => {
        if (res.data.code == 0) {
          this.shuiguofenleiOptions = res.data.data;
        }
      });
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.shuiguomingcheng != '') searchWhere.shuiguomingcheng = '%' + this.formSearch.shuiguomingcheng + '%';
        if (this.formSearch.shuiguofenlei != '') searchWhere.shuiguofenlei = this.formSearch.shuiguofenlei;
        if (this.formSearch.jijie != '') searchWhere.jijie = this.formSearch.jijie;
        ...
        if (this.curFenlei != '全部') searchWhere.shuiguofenlei = this.curFenlei;
        this.$http.get('shuiguoxinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/shuiguoxinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
